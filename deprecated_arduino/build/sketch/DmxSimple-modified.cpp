...
ISR(ISR_NAME,ISR_NOBLOCK) {
  ...
  while (1) {
    ...
    if (dmxState > dmxMax) {
      ...
      // do a reset or something? this is custom code for K2 Bar...works
      *dmxPort &= ~dmxBit;
      for (uint8_t i=0; i<11; i++) delayMicroseconds(8);
      *dmxPort |= dmxBit;
      delayMicroseconds(12);
      dmxSendByte(0);
...
/** Write to a DMX channel (renamed from "write" to "writeToBuffer")
 * @param address DMX address in the range 1 - 512
 */
void DmxSimpleClass::writeToBuffer(int address, uint8_t value)
{
...
/**
 * Asynchronous method that enables interrupts to write dmx data to output
 */
